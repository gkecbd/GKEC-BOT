...
module.exports.run = async function({ api, event }) {
  ...
  const botName = global.config.BOTNAME || "GKEC BOT";

 
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    await api.changeNickname(`[ ${botPrefix} ] • ${botName}`, threadID, api.getCurrentUserID());

    api.sendMessage("আস্সালামুআলাইকুম আমি GKEC BOT এখন তোমাদের সাথে আড্ডা দিব..!", threadID, () => {
...
➤𝐌𝐞𝐬𝐬𝐞𝐧𝐠𝐞𝐫: https://m.me/100090277697424
➤Mail: gkecbdinfo@gmail.com

❖⋆═══════════════════════⋆❖
          𝐁𝐨𝐭 𝐎𝐰𝐧𝐞𝐫 ➢ GKEC Tournament Hub`;
